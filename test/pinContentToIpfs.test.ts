...
describe("pinContentToIpfs", function () {
  ...
  test("should be able to pin to local", async function () {
    ...
    // setup ipfs mock
    mockNode.forPinRemoteLs().thenReturn([{service: "foo", endpoint: new URL("http://localhost:54321")}]);
    mockNode.forGet().thenReturn(toString(file));

    // setup table for testing
    const { meta } = await db.prepare("create table mytable (id int, file text);").all();
    const tableName = meta.txn?.name ?? "";

    const pinner = new Pinner(mockNode.ipfsOptions, "local");
    const processor = createProcessor(pinner.pin.bind(pinner), pinner.resolveCid.bind(pinner));
    const query = await processor`INSERT INTO ${tableName} (id, file) values (1, '${file}');`
    ...
    const { results: resultSet } = await db.prepare(`SELECT * FROM ${tableName};`).all<RowObject>();
  ...
  test("should be able to pin to remote", async function () {
    ...
    const pinner = new Pinner(mockNode.ipfsOptions, "http://remote.pinning.com");
